...
class CoreTests(unittest.TestCase):
    ...
    def test_box_page_links_grid_to_individual_details(self):
        ...
        self.assertIn('id="custom-tab"', page)
        self.assertIn("自由編成", page)
        nav = page.split('<nav class="tabs"', 1)[1].split("</nav>", 1)[0]
        self.assertEqual(nav.count("<button"), 10)
        ...
        self.assertIn("総合Tier表", page)
        self.assertIn("このアプリでの評価方法", page)
        self.assertIn("「理想個体」の定義", page)
        self.assertIn("外部での評価", page)
        self.assertIn("きのみで優れる点", page)
        self.assertIn("食材で優れる点", page)
        self.assertIn("スキルで優れる点", page)
